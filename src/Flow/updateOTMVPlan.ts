...
export default function prepareUpdateOTMVPlan(
  client: FlowClient,
): Resolver {
  const schema = client.describe().TacticalUpdatesService.TacticalUpdatesPort
    .updateOTMVPlan.input;//console.log(JSON.stringify(schema));
  ...
  return instrument<Values, Result>({
    ...
    (values, options) =>
      new Promise((resolve, reject) => {console.log(JSON.stringify(serializer(injectSendTime(values))));
